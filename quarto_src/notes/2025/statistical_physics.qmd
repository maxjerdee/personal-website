...
description: "Foundations of statistical physics from energy to entropy, and their links to model inference"
categories:
  - Physics
  - Inference
  - Review
...
image: ../../assets/images/notes/statistical_physics/statistical-physics.svg
...
We ground our exploration in the simple statistical setting of independent flips of a coin with probability $p$ of landing heads. As discussed in this [note](../2025/statistical_inference.html), the model likelihood of observing a sequence $\boldsymbol{s}$ with $n_H$ heads out of $n$ flips is

:::{style="overflow-x:auto;overflow-y:hidden;"}
$$\begin{aligned}
    P(\boldsymbol{s}|p) = p^{n_H}(1-p)^{n - n_H}. \end{aligned}$$ {#eq-coin-flip-likelihood}
:::

We define an physical model equivalent to this statistical one and explore how this physical perspective motivates alternative types of models and inference techniques. The presentation here is adapted from my [PhD thesis](../../assets/pdf/Thesis.pdf), alongside reviews of [statistical inference](../2025/statistical_inference.html) and [information theory](../2025/information_theory.html)   
...
:::{style="overflow-x:auto;overflow-y:hidden;"}
$$\begin{aligned}
    \Omega(E) = \binom{n}{E}. \end{aligned}$$ {#eq-Omega-E}
:::
...
statistical mechanics, the framework we adopt in this note. Under this
postulate the probability of observing any specific
configuration $\boldsymbol{s}$ is therefore 

:::{style="overflow-x:auto;overflow-y:hidden;"}
$$\begin{aligned}
    P(\boldsymbol{s}|E) = \binom{n}{E}^{-1}. \end{aligned}$$ {#eq-coin-flip-P-microcanonical}
:::
...
Many statistical models are framed in
...
:::{style="overflow-x:auto;overflow-y:hidden;"}
$$\begin{aligned}
    P(\boldsymbol{s},\tilde{\boldsymbol{s}}|E_T) &= \binom{n + \tilde{n}}{E_T}^{-1}. \end{aligned}$$ {#eq-coin-flip-joint-microcanonical}
:::
...
:::{style="overflow-x:auto;overflow-y:hidden;"}
$$\begin{aligned}
    S(E) = \log \Omega(E) = \log \binom{n}{E}, \text{ or }\, \tilde{S}(\tilde{E}) = \log \tilde{\Omega}(\tilde{E}) = \log \binom{\tilde{n}}{\tilde{E}}. \end{aligned}$$ {#eq-entropy-microcanonical-definition}
:::
...
:::{style="overflow-x:auto;overflow-y:hidden;"}
$$\begin{aligned}
    P(E|E_T) \propto e^{S(E) + \tilde{S}(\tilde{E})}. \end{aligned}$$ {#eq-probability-entropy}
:::
...
$$\begin{aligned}
    ...
    &\propto e^{\tilde{S}(\tilde{E})}. \end{aligned}$$ {#eq-P-s-E_T}
:::
...
:::{style="overflow-x:auto;overflow-y:hidden;"}
$$\begin{aligned}
    \tilde{S}(\tilde{E}) = \beta \tilde{E} + C, \quad \beta = \log \frac{1-p}{p}. \end{aligned}$$ {#eq-tilde-S-tilde-E}
:::
...
:::{style="overflow-x:auto;overflow-y:hidden;"}
$$\begin{aligned}
    \frac{\partial \tilde{S}}{\partial \tilde{E}} = \beta = \frac{1}{T}, \end{aligned}$$ {#eq-temperature-definition}
:::
...
:::{style="overflow-x:auto;overflow-y:hidden;"}
$$\begin{aligned}
    P(\boldsymbol{s}|\beta) = \frac{1}{Z(\beta)}e^{-\beta H(\boldsymbol{s})}, \quad Z(\beta) = \sum_{\boldsymbol{s}} e^{-\beta H(\boldsymbol{s})} \end{aligned}$$ {#eq-boltzmann-distribution}
:::
...
$$\begin{aligned}
    ...
    &= \langle E \rangle \end{aligned}$$ {#eq-average-energy-partition-function}
:::
...
$$\begin{aligned}
    ...
    &= p^{n_H}(1-p)^{n - n_H} \end{aligned}$$ {#eq-coin-flips-thermal}
:::

which we recognize as the model likelihood
[Eq. @eq-coin-flip-likelihood] for biased coin flips with
...
parameters $\boldsymbol{\theta}$. As discussed [here](../2025/statistical_inference.html), the posterior distribution of
...
:::{style="overflow-x:auto;overflow-y:hidden;"}
$$\begin{aligned}
    H(\boldsymbol{\theta}) = -\log P(\boldsymbol{x}|\boldsymbol{\theta}) \end{aligned}$$ {#eq-energy-posterior-relation}
:::
...
:::{style="overflow-x:auto;overflow-y:hidden;"}
$$\begin{aligned}
    P(\boldsymbol{s}') = \sum_{\boldsymbol{s}}P(\boldsymbol{s})P(\boldsymbol{s} \rightarrow \boldsymbol{s}'). \end{aligned}$$ {#eq-MCMC-stability}
:::
...
These Monte Carlo methods are very powerful, and are the workhorse behind much of Bayesian inference. In order to apply these
