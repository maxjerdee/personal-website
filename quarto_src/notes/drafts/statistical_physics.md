...
represent this configuration as a vector $\vec{s}$, where each
...
Figure [1](#fig:coin-flip-energies){reference-type="ref"
reference="fig:coin-flip-energies"}. The total energy of the system,
referred to as the *Hamiltonian*, is then the number of excited states
(or heads) $$\begin{aligned}
...
overall energy of the configuration $\vec{s}$ is equal to the number of
observed
heads, $H(\vec{s}) = n_H$.](max_dissertation//chapters//figures//chp1/coin-flip-energies.pdf){#fig:coin-flip-energies}

In an isolated system the energy
Eq. [\[eq:coin-flip-energy\]](#eq:coin-flip-energy){reference-type="eqref"
reference="eq:coin-flip-energy"} is always conserved, a fundamental law
...
system, so that only configurations $\vec{s}$ where $H(\vec{s}) = E$ are
...
states among the $n$ sites, $$\begin{aligned}
...
configuration $\vec{s}$ is therefore $$\begin{aligned}
    P(\vec{s}|E) = \binom{n}{E}^{-1}. \label{eq:coin-flip-P-microcanonical}\end{aligned}$$
...
Figure [2](#fig:coin-flip-ensemble){reference-type="ref"
reference="fig:coin-flip-ensemble"} illustrates this set up for the coin
flip example. Let $\tilde{\vec{s}}$ represent the configuration of the
thermal bath, containing $\tilde{n}$ spins and
energy $\tilde{E} = H(\tilde{\vec{s}})$ which counts the number of
excited states in the bath. Including our subsystem $\vec{s}$, the full
system $(\vec{s},\tilde{\vec{s}})$ then has $n + \tilde{n}$ total spins
and total energy $E_T = H(\vec{s},\tilde{\vec{s}}) = E + \tilde{E}$.
Since this total energy is conserved, the full
system $(\vec{s},\vec{s}')$ is uniformly distributed according to the
microcanonical ensemble
Eq. [\[eq:coin-flip-P-microcanonical\]](#eq:coin-flip-P-microcanonical){reference-type="eqref"
reference="eq:coin-flip-P-microcanonical"} of $E_T$ excited states
on $n + \tilde{n}$ sites, $$\begin{aligned}
    P(\vec{s},\tilde{\vec{s}}|E_T) &= \binom{n + \tilde{n}}{E_T}^{-1}. \label{eq:coin-flip-joint-microcanonical}\end{aligned}$$

Although each pair $(\vec{s},\vec{s}')$ with total energy $E_T$ is
equally likely, certain subsystem energies $E$ are more likely than
others. Figure [2](#fig:coin-flip-ensemble){reference-type="ref"
reference="fig:coin-flip-ensemble"} demonstrates this effect with two
example configurations. In panel (a) the excited states (\"H\") are all
contained within the thermal bath $\tilde{\vec{s}}$, meaning that the
subsystem $\vec{s}$ has its lowest possible energy $E = 0$ and the
...
(b).](max_dissertation//chapters//figures//chp1/coin-flip-ensemble.pdf){#fig:coin-flip-ensemble}

To quantify this tendency, we can count the number of overall
subsystem-bath configurations $(\vec{s},\tilde{\vec{s}})$ with subsystem
energy $E$. As in
Eq. [\[eq:Omega-E\]](#eq:Omega-E){reference-type="eqref"
reference="eq:Omega-E"}, there are $\Omega(E)$ microstates of the
subsystem $\vec{s}$ all share the same energy $E$. We refer to this
collection of microstates as a *macrostate* of the subsystem with
energy $E$. Figure [3](#fig:coin-flip-entropy){reference-type="ref"
reference="fig:coin-flip-entropy"} illustrates this grouping of
...
thermal bath $\tilde{\vec{s}}$ at energies $\tilde{E}$. Considering the
number of ways to arrange the $\tilde{E}$ excited states among the
$\tilde{n}$ sites of the bath, there are $$\begin{aligned}
...
![Example microstates of the subsystem $\vec{s}$ grouped into
...
observed.](max_dissertation//chapters//figures//chp1/coin-flip-entropy.pdf){#fig:coin-flip-entropy}
...
pairs $(\vec{s},\tilde{\vec{s}})$ with subsystem energy $E$ and bath
...
energy $E$ with probability $$\begin{aligned}
    P(E|E_T) &\propto \Omega(E)\tilde{\Omega}(\tilde{E}) \nonumber \\
    &\propto \Omega(E) \tilde{\Omega}(E_T - E). \end{aligned}$$ These
macrostate multiplicities can also be written using macrostate
*entropy*, defined as the logarithm[^2] $$\begin{aligned}
...
entropy $S(E) + \tilde{S}(\tilde{E})$ as $$\begin{aligned}
...
Figure [2](#fig:coin-flip-ensemble){reference-type="ref"
reference="fig:coin-flip-ensemble"}. This tendency to observe higher
...
reference="eq:probability-entropy"} approaches a certainty, a law of
physics.

We can similarly compute the distribution of subsystem
configurations $\vec{s}$, not just of its energy $E$. If we fix the
subsystem microstate, valid pairs $(\vec{s},\tilde{\vec{s}})$ correspond
to bath microstates $\tilde{\vec{s}}$ among
...
Eq. [\[eq:entropy-microcanonical-definition\]](#eq:entropy-microcanonical-definition){reference-type="eqref"
reference="eq:entropy-microcanonical-definition"}, we find the bath's
entropy $\tilde{S}(\tilde{E})$ is proportional to its energy (up to a
constant $C$) as $$\begin{aligned}
...
changes of entropy $\tilde{S}$ as $$\begin{aligned}
...
Comparing to Eq. [\[eq:P-s-E\_T\]](#eq:P-s-E_T){reference-type="eqref"
reference="eq:P-s-E_T"}, we then see that the probability of observing
any given subsystem $\vec{s}$ in this canonical ensemble is
$$\begin{aligned}
    P(\vec{s}|\beta) &\propto e^{\beta \tilde{E}}  \propto e^{-\beta E} \nonumber \\
    &\propto e^{-\beta H(\vec{s})},\end{aligned}$$ where we have used
that the total energy $E_T = E + \tilde{E}$ is conserved. In fact, by
similar arguments any system $\vec{s}$ in thermal equilibrium with a
large bath at inverse temperature $\beta$ follows this same *Boltzmann
distribution* (or \"Gibbs
distribution\") $P(\vec{s}) \propto e^{-\beta H(\vec{s})}$, which may
itself taken as the definition of the canonical ensemble of $\vec{s}$.
When $T = 0$, $\beta \rightarrow \infty$, the subsystem is stuck in its
ground state with the smallest possible energy $H(\vec{s})$. This aligns
...
Figure [2](#fig:coin-flip-ensemble){reference-type="ref"
reference="fig:coin-flip-ensemble"}. Although each individual microstate
...
Eq. [\[eq:probability-entropy\]](#eq:probability-entropy){reference-type="eqref"
reference="eq:probability-entropy"} for the distribution of energy $E$.
...
where we have defined the *free energy* $$\begin{aligned}
    F(E) = E - T S(E).\end{aligned}$$ The most likely energy $E$ to be
...
Boltzmann distribution as $$\begin{aligned}
    P(\vec{s}|\beta) = \frac{1}{Z(\beta)}e^{-\beta H(\vec{s})}, \quad Z(\beta) = \sum_{\vec{s}} e^{-\beta H(\vec{s})} \label{eq:boltzmann-distribution}\end{aligned}$$
...
energy relation
Eq. [\[eq:average-energy-partition-function\]](#eq:average-energy-partition-function){reference-type="eqref"
reference="eq:average-energy-partition-function"} of the physical system
indeed recovers the expected number of heads $$\begin{aligned}
...
and physics. In
Eq. [\[eq:coin-flips-thermal\]](#eq:coin-flips-thermal){reference-type="eqref"
reference="eq:coin-flips-thermal"} we described a physical system that
...
we have a generic model $P(\vec{x}|\vec{\theta})$ of data $\vec{x}$ with
parameters $\vec{\theta}$. As discussed in
...
parameters inferred from data $\vec{x}$ is by Bayes' law
$$\begin{aligned}
    P(\vec{\theta}|\vec{x}) &= \frac{P(\vec{x}|\vec{\theta})P(\vec{\theta})}{P(\vec{x})}.\end{aligned}$$

For a fixed observation $\vec{x}$, the corresponding physical system is
defined by the Hamiltonian $$\begin{aligned}
    H(\vec{\theta}) = -\log P(\vec{x}|\vec{\theta}) \label{eq:energy-posterior-relation}\end{aligned}$$
over the configuration space of possible parameters $\vec{\theta}$. If
we assume a uniform prior $P(\vec{\theta}) = 1$, the posterior
distribution over parameters is proportional to the Boltzmann
distribution of this system at unit temperature $\beta = 1$
$$\begin{aligned}
    P(\vec{\theta}|\vec{x}) \propto P(\vec{x}|\vec{\theta}) \propto e^{-H(\vec{\theta})}.\end{aligned}$$
In fact, if we normalize the Boltzmann distribution as $$\begin{aligned}
    P(\vec{\theta}|\vec{x}) = \frac{1}{Z(1)} e^{-H(\vec{\theta})},\end{aligned}$$
the partition function $Z(1)$ is equal to the Bayesian evidence
$$\begin{aligned}
    Z(1) = \sum_{\vec{\theta}} e^{-H(\vec{\theta})} = \sum_{\vec{\theta}} P(\vec{x}|\vec{\theta}) P(\vec{\theta}) = P(\vec{x}).\end{aligned}$$
...
maximizes $P(\vec{\theta}|\vec{x})$. Physically, this is akin to
...
Figure [2](#fig:coin-flip-ensemble){reference-type="ref"
reference="fig:coin-flip-ensemble"}, there may be many other less likely
...
Figure [2](#fig:coin-flip-ensemble){reference-type="ref"
reference="fig:coin-flip-ensemble"} the subsystem dynamically evolves.
If our subsystem $\vec{s}$ begins in a the low entropy ground state (a),
...
a random walk, where a move from state $\vec{s}$ to $\vec{s}'$ is made
with probability $P(\vec{s} \rightarrow \vec{s}')$. Under mild
conditions on the transition probabilities, this Markov chain will
converge to some equilibrium distribution $P(\vec{s})$. If the states of
the Markov chain are distributed according the equilibrium $P(\vec{s})$,
the final $\vec{s}'$ after the chain move $\vec{s} \rightarrow \vec{s}'$
will by definition share the equilibrium distribution $$\begin{aligned}
    P(\vec{s}') = \sum_{\vec{s}}P(\vec{s})P(\vec{s} \rightarrow \vec{s}'). \label{eq:MCMC-stability}\end{aligned}$$
We are then left with the problem of constructing a Markov chain which
has the particular equilibrium distribution $P(\vec{s})$ we are
interested in.
...
probabilities $P_{\text{prop}}(\vec{s} \rightarrow \vec{s}')$, we accept
each proposal with probability $$\begin{aligned}
    P_{\text{acc}}(\vec{s} \rightarrow \vec{s}') = \text{min}\left(1, \frac{P(\vec{s}')P_{\text{prop}}(\vec{s}' \rightarrow \vec{s})}{P(\vec{s})P_{\text{prop}}(\vec{s} \rightarrow \vec{s}')}\right).\end{aligned}$$
We will often use symmetric proposals
where $P_{\text{prop}}(\vec{s} \rightarrow \vec{s}') = P_{\text{prop}}(\vec{s}' \rightarrow \vec{s})$,
for which this acceptance probability simplifies to $$\begin{aligned}
    P_{\text{acc}}(\vec{s} \rightarrow \vec{s}') = \text{min}\left(1, \frac{P(\vec{s}')}{P(\vec{s})}\right) = \text{min}\left(1, e^{\beta(H(\vec{s}) - H(\vec{s}'))}\right).\end{aligned}$$
In terms of the system energy $H(\vec{s})$, we see that the algorithm
...
probability as $$\begin{aligned}
    ...
    \end{cases}\end{aligned}$$ where sum in the
case $\vec{s}' = \vec{s}$ accounts for all the rejected proposals to
states $\vec{s}'$ with probability $P(\vec{s}') < P(\vec{s})$. Summing
over these cases, now of states $\vec{s}$ that can produce a
state $\vec{s}'$, we can check the stability condition
Eq. [\[eq:MCMC-stability\]](#eq:MCMC-stability){reference-type="eqref"
reference="eq:MCMC-stability"} as $$\begin{aligned}
    ...
    &= P(\vec{s}')\sum_{\vec{s}} P_{\text{prop}}(\vec{s} \rightarrow \vec{s}') = P(\vec{s}').\end{aligned}$$

Therefore the desired distribution is a fixed point of the
Metropolis-Hastings algorithm, regardless of the choice of
proposals $P_{\text{prop}}(\vec{s} \rightarrow \vec{s}')$. However,
...
value.](max_dissertation//chapters//figures//chp1/MCMC-coin-flip.pdf){#fig:MCMC-coin-flip}

If we apply this to our simple coin flipping example,
Figure [4](#fig:MCMC-coin-flip){reference-type="ref"
reference="fig:MCMC-coin-flip"} shows how the number of flips changes
...
initial samples of Figure [4](#fig:MCMC-coin-flip){reference-type="ref"
reference="fig:MCMC-coin-flip"}.
...
[^2]: More generally defined as $S = k_B \log \Omega$
    ...
    relation
    Eq. [\[eq:temperature-definition\]](#eq:temperature-definition){reference-type="eqref"
    reference="eq:temperature-definition"}.
